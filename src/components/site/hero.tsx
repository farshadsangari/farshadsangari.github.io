...
export function Hero() {
  return (
    <section id="index" className="scroll-mt-24 border-b border-rule">
      <div className="grid lg:grid-cols-[minmax(0,1.65fr)_minmax(0,0.85fr)]">
        <div className="rise min-w-0 px-6 pb-14 pt-16 sm:px-10 lg:px-16 lg:pb-20 lg:pt-24">
          ...
          <h1 className="mt-7 text-balance font-display text-[2.75rem] font-semibold leading-[0.95] tracking-[-0.035em] sm:text-6xl lg:text-[4.5rem]">
            {profile.name}
          </h1>
          ...
          <p className="mt-6 max-w-xl leading-relaxed text-ink-soft">{profile.intro}</p>
          ...
          <dl className="mt-14 max-w-xl divide-y divide-rule border-t border-rule">
        ...
        <div className="relative hidden border-l border-rule bg-paper-deep lg:block">
          <img
            src={portrait}
            alt={`${profile.name}, AI software engineer and generative AI researcher`}
            width={720}
            height={900}
            className="h-full w-full object-cover object-top contrast-[1.05] saturate-[0.55]"
          />
          <div className="pointer-events-none absolute inset-0 bg-signal/10 mix-blend-multiply" />
          <p className="label-mono absolute bottom-5 left-5 right-5 text-paper/80">
            {profile.location}
          </p>
        </div>
